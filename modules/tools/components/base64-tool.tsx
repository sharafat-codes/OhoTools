...
function decodeB64(b: string) {
  return new TextDecoder().decode(
    Uint8Array.from(atob(b.trim()), (c) => c.charCodeAt(0)),
  );
}
...
export function Base64Tool() {
  ...
  function convert(text: string, m: Mode) {
    ...
    try {
      setOutput(m === "encode" ? encodeB64(text) : decodeB64(text));
      setError(null);
    } catch {
      setOutput("");
      setError(
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" onClick={() => { const s = "Hello, ToolPilot!"; setInput(s); convert(s, mode); }}>Try example</Button>
