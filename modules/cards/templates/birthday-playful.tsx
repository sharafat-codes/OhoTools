...
.bp-rain{position:absolute; top:-12%; font-size:clamp(1.6rem,5vw,2.4rem); animation-name:bp-fall; animation-timing-function:linear; animation-iteration-count:infinite; z-index:1;}
@keyframes bp-fall{0%{top:-12%; transform:rotate(0deg)}100%{top:115%; transform:rotate(30deg)}}
.bp-content{position:relative; z-index:10; padding:24px; max-width:90%;}
.bp-photo{display:block; margin:0 auto .6em; width:clamp(88px,23vw,124px); height:clamp(88px,23vw,124px); border-radius:50%; object-fit:cover;
  border:4px solid #fff; box-shadow:0 10px 30px rgba(0,0,0,.18); animation:bp-boing .8s .1s both;}
.bp-eyebrow{display:inline-block; font-size:clamp(1rem,3.4vw,1.3rem); font-weight:800; color:#fff; background:var(--bg2); padding:.35em .9em; border-radius:999px; box-shadow:0 6px 18px rgba(0,0,0,.15); animation:bp-boing .7s .15s both;}
.bp-name{margin:.2em 0 .25em; font-weight:900; letter-spacing:-.02em; line-height:1; font-size:clamp(3rem,13vw,6rem); color:var(--bg1);
  text-shadow:3px 3px 0 var(--accent); animation:bp-boing .8s .3s both, bp-wobble 3.5s ease-in-out .9s infinite;}
.bp-msg{font-size:clamp(1.05rem,3.6vw,1.4rem); line-height:1.55; font-weight:600; color:color-mix(in srgb, var(--bg1) 85%, #000); max-width:30ch; margin:0 auto; animation:bp-in .7s .5s both; text-wrap:balance;}
.bp-from{margin-top:1.1em; font-size:clamp(1rem,3vw,1.2rem); font-weight:800; color:var(--bg2); animation:bp-in .7s .7s both;}
